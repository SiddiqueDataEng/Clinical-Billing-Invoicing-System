...
Billing workflows integrated with services
...
## 🛠️ Technology Stack (2011)

### Database
- **Database Server**: Microsoft SQL Server 2008 R2 / 2012
- **Database Name**: ClinicalBillingDB
- **ORM**: ADO.NET
- **Migration Tool**: SQL Scripts

### Backend
- **Framework**: MSSQL, Stored Procedures
- **Language**: C# / VB.NET
...
```
07-Clinical-Billing-Invoicing-System/
├── README.md                          # This file
...
│
├── src/                               # Source code
│   ├── Clinical-Billing-Invoicing-System.sln            # Visual Studio solution
│   ├── Web/                          # Web application
...
cd 07-Clinical-Billing-Invoicing-System
```
...
sqlcmd -S localhost -d ClinicalBillingDB -i 02-CreateTables.sql
sqlcmd -S localhost -d ClinicalBillingDB -i 03-CreateStoredProcedures.sql
sqlcmd -S localhost -d ClinicalBillingDB -i 04-CreateTriggers.sql
sqlcmd -S localhost -d ClinicalBillingDB -i 05-CreateIndexes.sql
sqlcmd -S localhost -d ClinicalBillingDB -i 06-SeedData.sql
```

Or use the automated script:

```powershell
.\deployment\setup-database.ps1 -ServerName "localhost" -DatabaseName "ClinicalBillingDB"
```
...
```xml
<connectionStrings>
  <add name="ClinicalBillingDB" 
       connectionString="Server=localhost;Database=ClinicalBillingDB;Integrated Security=true;" 
...
```powershell
cd src
msbuild Clinical-Billing-Invoicing-System.sln /p:Configuration=Release
```
...
```powershell
.\deployment\setup-iis.ps1 -SiteName "Clinical-Billing-Invoicing-System" -Port 8080
```
...
<connectionStrings>
  <add name="ClinicalBillingDB" 
       connectionString="Server=YOUR_SERVER;Database=ClinicalBillingDB;User Id=YOUR_USER;Password=YOUR_PASSWORD;" 
       providerName="System.Data.SqlClient" />
</connectionStrings>
```
...
```xml
<appSettings>
  <add key="ApplicationName" value="Clinical-Billing-Invoicing-System" />
  <add key="PageSize" value="20" />
  <add key="SessionTimeout" value="30" />
  <add key="EnableLogging" value="true" />
  <add key="LogPath" value="C:\Logs\Clinical-Billing-Invoicing-System" />
</appSettings>
```
...
```bash
# Build Docker image
docker build -t Clinical-Billing-Invoicing-System:latest .
...
az deployment group create --resource-group rg-Clinical-Billing-Invoicing-System --template-file main.bicep
...
- **Logs**: Check `C:\Logs\Clinical-Billing-Invoicing-System`
...
SELECT * FROM sys.dm_exec_sessions WHERE database_id = DB_ID('ClinicalBillingDB')

-- Check long-running queries
SELECT * FROM sys.dm_exec_requests WHERE database_id = DB_ID('ClinicalBillingDB')
```
